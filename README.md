...
| モデル | パラメータ | 必要VRAM | HuggingFace | 備考 |
|---|---|---|---|---|
...
| [SALMONN-13B](https://huggingface.co/tsinghua-ee/SALMONN) | 13B | ~26 GB | tsinghua-ee/SALMONN | ⚠️ BEATsチェックポイント要手動取得 |
...
> デフォルトでは `/workspace/.cache/huggingface`（ワークスペース内）にキャッシュが保存されます。追加設定は不要です。  
> 大容量の外部ストレージを使いたい場合は `.devcontainer/.env.local.example` を参照してください。
...
**SALMONN-13B（⚠️ 現在未対応 — 手動セットアップが必要）:**

> SALMONN-13B は BEATs の事前学習済みチェックポイント（`BEATs_iter3_plus_AS2M_finetuned_on_AS2M_cpt2.pt`）を別途ダウンロードし、環境変数 `SALMONN_BEATS_PATH` で指定する必要があります。
> チェックポイントは [SALMONN GitHub](https://github.com/bytedance/SALMONN/tree/salmonn) の手順に従い手動取得してください。
> ファイルが用意できたら以下のコマンドで起動できます：
>
> ```bash
