...
_SERVED_MODEL_NAME = "audex_audio"
_TEXT_VOCAB_SIZE = 131072
_AUDIO_PLACEHOLDER_TOKEN_IDS = {29, 30, 31}
_ALLOWED_TOKEN_IDS = [
    i for i in range(_TEXT_VOCAB_SIZE) if i not in _AUDIO_PLACEHOLDER_TOKEN_IDS
]
...
class NemotronLabsAudexModel(AudioModel):
    def __init__(self, device: str = "cuda") -> None:
        self._vllm_url = os.environ.get("AUDEX_VLLM_URL", "http://audex-vllm:9999")

    @property
    def display_name(self) -> str:
        return "Nemotron-Labs-Audex-30B-A3B"
    ...
    def load(self) -> None:
        logger.info("Waiting for Audex vLLM server at %s ...", self._vllm_url)
        deadline = time.time() + _VLLM_READY_TIMEOUT
        while time.time() < deadline:
            try:
                r = httpx.get(f"{self._vllm_url}/health", timeout=5.0)
                if r.status_code == 200:
                    logger.info("Audex vLLM server ready.")
                    return
            except Exception:
                pass
            time.sleep(5)
    ...
    def run_inference(
        ...
        payload = {
            ...
            "extra_body": {
                "top_k": 0,
                "allowed_token_ids": _ALLOWED_TOKEN_IDS,
        ...
        with httpx.Client(timeout=300.0) as client:
            r = client.post(f"{self._vllm_url}/v1/chat/completions", json=payload)
        r.raise_for_status()
